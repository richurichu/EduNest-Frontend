...
function QuizQuestion({ currentquestionid, questionArray, handleindexchange, setAttempted, openModal, setOpenModal, timeOver, setTimeOver, quizId, attempted }) {
  ...
  const onsubmitQuiz = async () => {
    try {
      ...
      toast.error("You have already  submitted. Redirecting to Quiz page", {
  ...
  return (
    <>
      <div className="w-full  sm:w-3/4 lg:w-1/2 p-8  sm:mt-16 mx-auto shadow-2xl bg-gradient-to-r from-blue-200 via-purple-300 to-pink-300">
        ...
        <div className="space-y-4 p-4">
          {currentquestion && currentquestion.options.map((option, index) => (<div onClick={() => handleOptionSelect(currentquestion.id, option.id)} className={`bg-gray-200 p-6 rounded-lg ${isOptionSelected(currentquestion.id, option.id) ? 'bg-green-200' : ''
            }`}>
        ...
        <div className="flex justify-between mt-4">
          ...
          <button onClick={clearSelectedOptions} className="px-4 py-2 bg-gray-500 text-white rounded-lg ml-4 hover:bg-sky-600">
            Clear
          </button>
